...
func welcome() {
	...
	fmt.Println("-------------------------------------------------------")
	fmt.Printf("Silakan pilih menu di bawah ini\n1. Login sebagai admin\n2. Login sebagai guest\n")
	fmt.Printf("Tekan tombol sembarang untuk keluar aplikasi\n")
	fmt.Scanln(&pilih)
	switch pilih {
	case 1:
		login_admin()
...
func akses_admin() {
	...
	case 2:
		...
		fmt.Scanln(&pilih2)
		for (pilih2 > len(daftar.nama)) || (pilih2 < 0) {
	...
	case 3:
		...
		fmt.Scanln(&pilih2)
		for (pilih2 >= len(daftar.nama)) || (pilih2 < 0) || daftar.nama[pilih2] == "" {
			fmt.Println("Mohon maaf data yang anda masukkan tidak ada")
			fmt.Scanln(&pilih2)
		}
		terhapus := pilih2 - 1
...
func urut() {
	...
	switch pilihan {
	case 1:
		fmt.Println("Urutkan secara: 1. Menaik 2. Menurun")
		fmt.Scanln(&pilihan2)
		switch pilihan2 {
		case 1:
			urutbyusiaNaik()
...
func edit(index *int) {
	...
	fmt.Printf("\n2. Usia : %s\n", daftar.usia[*index])
	fmt.Printf("3. Jenis Kelamin : %s", daftar.jenisK[*index])
	...
	fmt.Printf("\n7. Pekerjaan yang dipilih : %s\n", daftar.kerjaan[*index])
	fmt.Println("----------------------------------------------------------")
	fmt.Printf("Masukkan angka bagian yang akan diedit:\n")
	fmt.Scanln(&pilihan)
...
func pendaftaran() {
	...
	fmt.Println("Tempat tanggal lahir : ")
	fmt.Print("Tempat lahir : ")
...
func carinama() {
	...
	var nama tabnama
	fmt.Println("Tulis nama yang akan anda cari")
...
func simpan() {
	...
	if cek2 {
		...
		fmt.Println("Selamat, data yang anda masukkan berhasil terdaftar!")
	}
...
func view() {
	var eof int
	fmt.Printf("\n----------------------------\n")
	fmt.Printf("SELAMAT!!! DATA BERHASIL DIREKAM\nBerikut ini ulasan dari data yang anda masukkan,\n")
	fmt.Println("----------------------------")
